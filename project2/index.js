...
const coinsEvent = new CustomEvent('evet:coins', undefined);
$(()=>{
    $.get(coinlistURL,(data)=>{
        const myReg = new RegExp(/^\d/);
        for(let index = 0; index < data.length; index++){
            if(!myReg.test(data[index].id)){
    ...
    const moveToCoins = ()=>{
        $('#content').empty();
        $('#content').append(firstImg).trigger('event:coins');
    ...
    $("a:first").on('click', ()=>{
        $('#content').empty();
        $('#content').append(mainPage).trigger('event:coins');
